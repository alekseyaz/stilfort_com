...
/** Класс функционала eip */
	class EditInPlace {
		...
		public function eip_quick_add() {
			...
			if ($this->useHierarchy($objectType, getRequest('force-hierarchy'))) {
				$permissions = permissionsCollection::getInstance();
				if ($parentElementId) {
					$auth = Service::Auth();
					$userId = $auth->getUserId();
		...
		/**
		 * Определяет нужно ли создать страницу
		 * @param iUmiObjectType $objectType тип объекта
		 * @param null|int|string $forceHierarchy
		 * @return bool
		 */
		private function useHierarchy(iUmiObjectType $objectType, $forceHierarchy) {
			return (is_null($forceHierarchy) || (int) $forceHierarchy !== 0) && !in_array($objectType->getModule(), ['data', 'banners']);
		}
	}
