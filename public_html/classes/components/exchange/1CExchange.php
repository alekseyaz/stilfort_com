<?php
	...
	class OneCExchange {
		...
		protected function saveIncomingFile() {
			...
			$this->writeLog('extension: '.$extension);
		...
		protected function saveFile($fileName, $fileContent = '') {
			...
			$this->writeLog('1c_latest_catalog-file '. $fileName);
		...
		protected function importCommerceML() {
			$this->writeLog('start: importCommerceML');
			...
			$this->writeLog('end: importCommerceML');
		...
		public function writeLog($str = '', $file_name = 'exchange_log'){
			//$date = date("Y-m-d H:i:s");
			list($usec, $sec) = explode(" ", microtime());
			$date = date("Y-m-d H:i:s", $sec).substr($usec, 1, 7);
			
			$clientIP = getServer('REMOTE_ADDR');
			$path = CURRENT_WORKING_DIR . '/classes/components/exchange/logs/';
			if (!is_dir($path)) {
				mkdir($path, 0700, true);
			}
			$file = $path.date("Y.m.d").'_'.$file_name.'.txt';
			$fp = fopen($file, 'a+');
			fwrite($fp, $date.'|'.$clientIP.'||'.$str . "\r\n");
			fclose($fp);
		}
	}
